...
checkpoint_file=tf.train.latest_checkpoint("./Log_data/2cls_alexnet_20180322_sensi_half_448_l2_20103221743352011")
print('checkpoint:')
...
def run_testing():
	...
	with tf.Graph().as_default():
		...
		with slim.arg_scope(vgg.vgg_arg_scope()):
			...
			logits,end_points=alexnet_448.alexnet_v2(images,num_classes=gd.NUM_CLASSES,is_training=False)
		...
		with tf.Session(config=config) as sess:
			...
			csvfile=open(gd.DIR_DESCRIPTION+"/2cls_alexnet_analysis2_withnocolorchange_wrongprediction_182.csv","wb")
			writer=csv.writer(csvfile)
			writer.writerow(['labels','prediction','filename'])

			file_name2='/no_color_change.csv'
			...
			#while not coord.should_stop():
				...
				print ("%d : %d ,%d, max_probility: %f" %(step,labels_out[0],prediction_out[0],probability_max[0]))
				writer2.writerow([labels_out[0],prediction_out[0],probability_max[0]])
				# print("softmax:")
				# print(softmax_out)
				# print("probability:")
				# print(probability_max[0])
				#print(labels_out[0])
